    ...
    <div class="max-w-7xl mx-auto space-y-6 animate-fade-in pb-12">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
        ...
        <div class="flex items-center gap-4">
          ...
          <select 
            ...
            (change)="loadQueue()"
            class="px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-lg text-sm bg-surface text-text-main focus:ring-2 focus:ring-primary focus:border-primary transition-all">
            <option value="all">All Group Tickets</option>
            <option value="unassigned">Unassigned Only</option>
            <option value="overdue">SLA Breached / Overdue</option>
      ...
      <div class="bg-card-white dark:bg-surface rounded-xl border border-table-dark-gray dark:border-gray-800 shadow-sm overflow-hidden">
        @if (loading()) {
          ...
          <div class="py-12 text-center text-text-light">
            You are not managing any groups.
          </div>
        } @else {
          <div class="overflow-x-auto">
            <table class="w-full text-left text-sm text-text-main">
              <thead class="text-xs uppercase bg-table-light-gray dark:bg-gray-800 text-text-light dark:text-text-muted border-b border-table-dark-gray dark:border-gray-800">
                <tr>
                  <th scope="col" class="px-6 py-4 font-semibold">Ticket ID</th>
              ...
              <tbody class="divide-y divide-table-dark-gray dark:divide-gray-800">
                @for (ticket of tickets(); track ticket.id) {
                  <tr class="hover:bg-table-light-gray dark:hover:bg-gray-800/50 transition-colors">
                    <td class="px-6 py-4 font-medium">{{ ticket.ticketNumber }}</td>
                    <td class="px-6 py-4">
...
export class QueueComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // First get the groups the lead manages
    this.teamLeadService.getLeadDashboard().subscribe({
      ...
      error: (err) => {
        console.error('Error loading lead dashboard for groups', err);
        this.loading.set(false);
      }
    });
  }
  ...
  loadQueue() {
    ...
    this.loading.set(true);
    this.teamLeadService.getGroupQueue(this.selectedGroupId, this.queueType, this.page(), this.pageSize()).subscribe({
      next: (res) => {
        this.tickets.set(res.tickets.items);
        this.totalCount.set(res.tickets.totalCount);
        this.loading.set(false);
      },
      error: (err) => {
        console.error('Error loading group queue', err);
        this.loading.set(false);
      }
    });
  }
