...
- **Role management** — Create, update, soft-delete named roles; system roles are protected from deletion
...
- **User ↔ Role assignment** — Assign/remove individual roles or bulk-replace a user's full role set
...
|-------|---------|-------------|
| `GetAllRolesQuery` | `IReadOnlyList<RoleDto>` | All non-deleted roles |
| `GetRoleByIdQuery` | `RoleDto` | Single role |
