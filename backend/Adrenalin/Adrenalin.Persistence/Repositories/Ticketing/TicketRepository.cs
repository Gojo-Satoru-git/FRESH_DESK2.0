...
{
	private static readonly Func<AdrenalinDbContext, string, Task<Ticket?>> CompiledGetByTicketNumber = 
		EF.CompileAsyncQuery((AdrenalinDbContext context, string ticketNumber) =>
			context.Tickets
				...
				.AsSplitQuery()
				.FirstOrDefault(x => x.TicketNumber == ticketNumber && !x.IsDeleted));
	...
	public Task<Ticket?> GetByTicketNumberAsync(string ticketNumber, CancellationToken cancellationToken = default)
	{
		return CompiledGetByTicketNumber(_context, ticketNumber);
	}
    ...
    public async Task<bool> TryClaimTicketAsync(
        Guid ticketId,
        Guid agentId,
        byte[] rowVersion,
        CancellationToken ct = default)
    {
        var rowsAffected = await _context.Database.ExecuteSqlRawAsync(
            "UPDATE ticket.tickets SET assigned_agent_id = {0}, updated_at = {1}, row_version = gen_random_bytes(16) WHERE id = {2} AND assigned_agent_id IS NULL AND row_version = {3} AND is_deleted = false",
            agentId, DateTimeOffset.UtcNow, ticketId, rowVersion, ct);
	...
	public async Task<string> GenerateTicketNumberAsync(CancellationToken cancellationToken = default)
	{
		...
		// Use the PostgreSQL sequence to guarantee unique atomic increment across concurrent consumers
		var nextSequence = await _context.Database.SqlQueryRaw<long>("SELECT nextval('ticket.ticket_number_seq') AS \"Value\"").FirstOrDefaultAsync(cancellationToken);
    ...
    CancellationToken ct = default)
    {
        ...
        if (groupId.HasValue && groupId != ticket.GroupId)
        {
            ticket.AssignGroup(
        ...
        if (agentId.HasValue)
        {
            ticket.AssignAgent(
                ...
                notes: notes ?? "Auto-assigned via automation rules");
        }
        else if (clearAgentIfNull || (groupId.HasValue && groupId != ticket.GroupId))
        {
            // Respect unassigned queue: clear the agent if moving to a new group and no agent is provided,
            // or if explicitly requested to clear.
            ticket.UnassignAgent(triggeredBy, notes);
        }
    }
    ...
    public async Task<bool> IsUserInternalAsync(Guid userId, CancellationToken cancellationToken = default)
    {
        var internalRoles = new[] { "admin", "manager", "pmo", "team_lead", "senior_agent", "junior_agent" };
        return await (from ur in _context.UserRoles
                       join r in _context.Roles on ur.RoleId equals r.Id
                       where ur.UserId == userId && internalRoles.Contains(r.Name.ToLower()) && !ur.IsDeleted && !r.IsDeleted
                       select ur.Id)
                      .AnyAsync(cancellationToken);
    }
