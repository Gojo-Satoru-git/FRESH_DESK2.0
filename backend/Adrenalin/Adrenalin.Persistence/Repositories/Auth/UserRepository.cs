...
{
    private static readonly Func<AdrenalinDbContext, Guid, IAsyncEnumerable<string>> CompiledGetUserPermissions =
        EF.CompileAsyncQuery((AdrenalinDbContext db, Guid userId) =>
            db.UserRoles
                .Where(x => x.UserId == userId && !x.IsDeleted)
                .SelectMany(x => x.Role.RolePermissions)
                .Where(x => !x.IsDeleted)
                .Select(x => x.Permission.Resource + ":" + x.Permission.Action)
                .Distinct());
        ...
        CancellationToken cancellationToken)
    {
        ...
        await foreach (var p in CompiledGetUserPermissions(_db, userId).WithCancellation(cancellationToken))
        {
            results.Add(p);
        }
        return results;
